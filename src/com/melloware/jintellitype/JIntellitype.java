...
public final class JIntellitype implements JIntellitypeConstants {
     ...
     * <p>
     * But the possibility of creation of more instance is only before the instance
     * is created. Since all code defined inside getInstance method is in the
     * synchronized block, even the subsequent requests will also come and wait in
     * the synchronized block. This is a performance issue. The same can be solved
     * using double-checked lock. Following is the implementation of Singleton with
     * lazy initialization and double-checked lock.
     * <p>
     * 
    ...
    /**
     * Registers a Hotkey with windows. This combination will be responded to by all
     * registered HotKeyListeners. Uses the JIntellitypeConstants for MOD, ALT,
    ...
    /**
     * Registers a Hotkey with windows. This combination will be responded to by all
     * registered HotKeyListeners. Use the Swing InputEvent constants from
     * java.awt.InputEvent.
     * <p>
     * 
     * @param identifier
     *            a unique identifier for this key combination
     * @param modifier
     *            InputEvent.SHIFT_MASK, InputEvent.ALT_MASK, InputEvent.CTRL_MASK,
    ...
    /**
     * Registers a Hotkey with windows. This combination will be responded to by all
     * registered HotKeyListeners. Use the identifiers CTRL, SHIFT, ALT and/or WIN.
     * <p>
     * 
     * @param identifier
     *            a unique identifier for this key combination
    ...
    public void registerHotKey(int identifier, String modifierAndKeyCode) {
        ...
        String strKey = split[split.length - 1];
        if (strKey.equalsIgnoreCase("UP")) {
            registerHotKey(identifier, mask, UP_ARROW_KEY);
        } else if (strKey.equalsIgnoreCase("DOWN")) {
            registerHotKey(identifier, mask, DOWN_ARROW_KEY);
        } else if (strKey.equalsIgnoreCase("LEFT")) {
            registerHotKey(identifier, mask, LEFT_ARROW_KEY);
        } else if (strKey.equalsIgnoreCase("RIGHT")) {
            registerHotKey(identifier, mask, RIGHT_ARROW_KEY);
        } else {
    ...
    /**
     * Checks to make sure the OS is a Windows flavor and that the JIntellitype DLL
     * is found in the path and the JDK is 32 bit not 64 bit. The DLL currently only
     * supports 32 bit JDK.
     * <p>
     * 
    ...
    /**
     * Swing modifier value to Jintellipad conversion. If no conversion needed just
     * return the original value. This lets users pass either the original
    ...
    /**
     * Checks if there's an instance with hidden window title = appName running Can
     * be used to detect that another instance of your app is already running (so
     * exit..)
     * <p>
     * 
     * @param appName
